		...
		try {
	        ...
	        for(Invoice i: invoices) {
				if (i.getUserId().equals(user.getUserId().toString()) && i.getStatus().toLowerCase().equals("pending")) {
					Reservation reservation = new ReservationRetriever().retrieveReservationById(i.getReservationId());
					...
					table.getTableModel().addRow(i.getId(), i.getReservationId(), String.format("$%.2f", Double.parseDouble(i.getTotalAmount())), formattedDueDate);
				}
			}
